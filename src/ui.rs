...
use ratatui::{
    ...
    text::{Line, Span},
    widgets::{
        Block, Borders, Clear, List, ListItem, ListState, Paragraph, Row, Sparkline, Table, Tabs,
        Wrap,
    },
};
...
pub fn draw_help_popup(f: &mut Frame, search_scope: &SearchScope) {
    ...
    let rows = vec![
        ...
        Row::new(vec!["  [Tab]".to_string(), scope_label]).style(key_style),
